...
@section('content')
   ...
   <!-- Page header -->
        <div class="page-header page-header-light">
            <div class="page-header-content header-elements-md-inline">
                <div class="page-title d-flex">
                    <h4><span class="font-weight-semibold">Master Data</span> - Branch</h4>
                    <a href="#" class="header-elements-toggle text-default d-md-none"><i class="icon-more"></i></a>
                </div>

                <div class="header-elements d-none">
                    @permission('create_branch')
                    <a href="{{route('branch.create')}}" class="btn btn-labeled btn-labeled-left bg-primary">New Branch <b><i class="icon-add"></i></b></a>
                    @endpermission
                </div>
            </div>
    ...
    <div class="content">
        <div class="card">
            <table class="table table-hover table-bordered table-xxs datatable-select-checkbox" id="data-table" data-url="{{route('branch.index')}}">
                <thead>
                <tr>    
...
var table = $('#data-table').DataTable({
    ...
    $(".dataTables_filter").prepend("<select class='select' id='table-action'>" +
                    ...
                    @if(\Entrust::can('delete_branch'))"<option value='2' disabled>Delete</option>" +@endif
                    "</select>"
            );
</script>
@endpush
@section('globaldatatables')
    @include('global.datatables-branch')
@endsection